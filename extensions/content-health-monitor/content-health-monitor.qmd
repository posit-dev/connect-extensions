...
import globals  # Import shared globals
...
client = None
if not globals.show_instructions:
    try:
        ...
        globals.instructions.append(f"<b>Error initializing Connect client:</b> {str(e)}")
...
if not globals.show_instructions and client:
    try:
...
# (globals.show_instructions=False already tells us env vars are set and client was initialized)
if not globals.show_instructions:
    try:
...
if globals.show_instructions:
    instructions_html = "".join(f"<div style='margin-bottom: 10px;'>{instruction}</div>" for instruction in globals.instructions)
else:
    instructions_html = None

# Create all HTML components up front based on state
html_components = {
    'instructions': create_instructions_box(instructions_html) if globals.show_instructions else None,
    ...
    'no_results': create_no_results_box() if not (globals.show_instructions or show_error or 
