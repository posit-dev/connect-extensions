...
def get_env_var(var_name, description=""):
    ...
    if not value:
        ...
        instruction = (
            ...
            f"and click <b>Add Variable</b>. When you're done, click <b>Refresh Report</b> to continue."
        )
        if description:
...
def create_error_box(guid, error_msg):
    ...
    <div style="{CSS_BOX_STYLE.format(border=error['border'], background=error['background'])}"> 
        ...
        <div style="{CSS_FOOTER_STYLE}">
            Please check that your MONITORED_CONTENT_GUID environment variable contains a valid content identifier.
        </div>
    </div>
    """
...
about_content = """<div>
This report uses the publisher's API key to monitor one piece of content, pings the monitored content but does not 
validate any content functionality, and reports the results. If run on a schedule, reports will be emailed when the 
monitored content reports a failure.
</div>"""
